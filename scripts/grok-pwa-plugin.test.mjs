...
test("platform chrome overwrites share-card metas and always sets og:title", () => {
  ...
  const out = injectGrokPwaHead(html, {
    appName: "Wild Race",
    cwd: mkdtempSync(join(tmpdir(), "grok-og-title-fallback-")),
  });
...
test("document title entities are not double-escaped on og:title", () => {
  const out = injectGrokPwaHead(
    ...
    { cwd: mkdtempSync(join(tmpdir(), "grok-og-document-title-")) },
  );
...
test("streaming injector matches </HEAD> case-insensitively", () => {
  const injector = createHeadInjector({
    appName: "Wild Race",
    cwd: mkdtempSync(join(tmpdir(), "grok-og-stream-title-")),
  });
  const chunks = [
...
test("uses the app name in the injected title tag", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", {
    appName: "Wild Race",
    cwd: mkdtempSync(join(tmpdir(), "grok-og-app-title-")),
  });
...
test("names the install page from host slug", () => {
  assert.equal(appNameFromHost("localhost:8080"), "COT Terminal");
  assert.equal(appNameFromHost("172.17.154.217:8080"), "COT Terminal");
...
test("uses COT Terminal for local and preview manifests", () => {
  const manifest = JSON.parse(renderWebManifest("localhost:8080"));
  assert.equal(manifest.name, "COT Terminal");
  assert.equal(manifest.short_name, "COT Terminal");
});
